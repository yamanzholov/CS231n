...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
        probabilities[j] = np.exp(scores[j]) / scores_sum
        dW[:, j] += (probabilities[j] - (j == y[i])) * X[i]
    sample_loss = -np.log(probabilities[y[i]])
    loss += sample_loss
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  probabilities = np.exp(scores) / scores_sum
  i = np.zeros_like(probabilities)
  i[np.arange(num_train), y] = 1
  dW = X.T.dot(probabilities - i)
