...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in range(num_test):
      for j in range(num_train):
        ...
        test_image = X[i]
        L2_dist = np.sqrt(np.sum(np.square(np.subtract(train_image, test_image))))
        dists[i, j] = L2_dist
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test):
      ...
      #######################################################################
      test_image = X[i]
      dists[i, :] = np.sqrt(np.sum(np.square(np.subtract(self.X_train, test_image)), axis = 1))
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      for t in closest_y:
            ...
            if count_dict[t] > max_count:
                max_count = count_dict[t]
                max_class = t
